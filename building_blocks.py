...
class Instance_block(nn.Module):
    ...
    def forward(self, phi, X_old, X_prv, Y, idx):
        X_out = torch.zeros_like(X_old).to(self.dev)
        ...
        idx += 1
        phi_out = phi - self.phi_step * (4 * phi @ (torch.transpose(phi, 0, 1) @ phi - torch.eye(self.p).to(self.dev)) - 2 * (Y[:, : idx].clone() - phi @ X_out[:, : idx]) @ torch.transpose(X_out[:, : idx], 0, 1))
...
class Constraint_block(nn.Module):
    ...
    def __init__(self, N, p, tr, steps, phi_steps, threshs, dev):
        super(Constraint_block, self).__init__()
        self.N = N
        self.p = p
        self.dev = dev
        layers = [Instance_block(self.p, tr, steps[i], phi_steps[i], threshs[i], self.dev) for i in range(self.N)]
